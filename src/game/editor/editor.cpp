...
void CEditor::DoMapEditor(CUIRect View)
{
	...
	// Shrink the map canvas so it doesn't overlap the touch toolbar at the bottom.
	// This prevents the map from consuming taps intended for toolbar buttons.
	const float AndroidToolbarH = 64.0f;
	View.HSplitBottom(AndroidToolbarH, &View, nullptr);

	// RMB mode: remap single touch LMB → RMB so the user can erase/open context menus.
	if(m_AndroidRmbMode)
	{
		const bool TouchActive = !Input()->TouchFingerStates().empty();
		Ui()->ForceMouseButton(0, false);       // suppress LMB
		Ui()->ForceMouseButton(1, TouchActive); // inject RMB when finger is on screen
	}
#endif
...
void CEditor::RenderAndroidTouchToolbar()
{
	...
	// Toolbar dimensions — must match AndroidToolbarH in DoMapEditor
	const float BtnH = 56.0f;
	const float Margin = 4.0f;
	const float TotalH = BtnH + Margin * 2.0f; // = 64.0f
	...
	Graphics()->TextureClear();
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.08f, 0.08f, 0.08f, 0.82f);
	...
	// IDs for button hit-testing (static — unique address per button)
	static int s_LmbBtn, s_RmbBtn, s_PickBtn, s_GuiBtn;
	static int s_ZoomOutBtn, s_ZoomResetBtn, s_ZoomInBtn;

	const float BtnW  = 72.0f;
	const float ZoomW = 52.0f;

	auto SplitBtn = [&](float W, CUIRect &OutBtn)
	{
		Bar.VSplitLeft(W, &OutBtn, &Bar);
	...
	SplitBtn(BtnW, Btn);
	{
		const bool Active = !m_AndroidRmbMode; // LMB is default when RMB is off
		if(DoButton_Ex(&s_LmbBtn, "LMB", Active ? -1 : 0, &Btn,
			BUTTONFLAG_LEFT, "Left mouse button mode (draw)", IGraphics::CORNER_ALL, 13.0f))
		{
			m_AndroidRmbMode = false;
		}
	}
	...
	SplitBtn(BtnW, Btn);
	{
		if(DoButton_Ex(&s_RmbBtn, "RMB", m_AndroidRmbMode ? -1 : 0, &Btn,
			BUTTONFLAG_LEFT, "Right mouse button mode (erase / context menu)", IGraphics::CORNER_ALL, 13.0f))
		{
			m_AndroidRmbMode = !m_AndroidRmbMode;
		}
	}
	...
	if(DoButton_Ex(&s_ZoomResetBtn, "1:1", 0, &Btn,
		BUTTONFLAG_LEFT, "Reset zoom to 1:1", IGraphics::CORNER_NONE, 12.0f))
		m_QuickActionResetZoom.Call();
	...
	// ── GUI toggle — right-aligned ─────────────────────────────────────────
	{
		...
		GuiBtn.w = BtnW;
		GuiBtn.h = Bar.h;
		if(DoButton_Ex(&s_GuiBtn, "GUI", m_GuiActive ? -1 : 0, &GuiBtn,
			BUTTONFLAG_LEFT, "Show/hide editor GUI (panels)", IGraphics::CORNER_ALL, 13.0f))
