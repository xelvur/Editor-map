...
{
	// Handle two-finger touch gestures (pinch-to-zoom + pan) for Android/mobile
#if defined(CONF_PLATFORM_ANDROID)
	MapView()->HandleTouchGestures();

	const int NumFingers = (int)Input()->TouchFingerStates().size();

	// ── Gesture vs drawing conflict fix ────────────────────────────────────
	// SDL maps finger-1 to mouse-button-1 automatically. If the user starts a
	// pinch (2 fingers), the first finger already triggered a LMB press on the
	// previous frame and may have placed a tile. We prevent this by:
	//   1. Blocking LMB for the first 2 frames of any new single touch
	//      (gives time for a second finger to appear and be detected as a gesture)
	//   2. Keeping LMB blocked for a 12-frame cooldown after any multi-touch gesture
	static int s_SingleTouchFrames = 0;  // consecutive frames with exactly 1 finger
	static int s_GestureCooldown   = 0;  // frames remaining after gesture ends
	static int s_PrevNumFingers    = 0;

	if(NumFingers >= 2)
	{
		// Active multi-touch gesture: block all drawing
		Ui()->ForceMouseButton(0, false);
		Ui()->ForceMouseButton(1, false);
		s_GestureCooldown   = 12;
		s_SingleTouchFrames = 0;
	}
	else if(s_GestureCooldown > 0)
	{
		// Just finished a gesture: still block drawing during cooldown
		Ui()->ForceMouseButton(0, false);
		Ui()->ForceMouseButton(1, false);
		s_GestureCooldown--;
		s_SingleTouchFrames = 0;
	...
	{
		if(s_PrevNumFingers == 0)
			s_SingleTouchFrames = 0; // fresh touch
		s_SingleTouchFrames++;

		if(s_SingleTouchFrames < 3)
		{
			// Block drawing for first 2 frames — waiting to confirm it's
			// not the start of a pinch gesture
			Ui()->ForceMouseButton(0, false);
			Ui()->ForceMouseButton(1, false);
		}
	}
	...
	s_PrevNumFingers = NumFingers;

	// Lock mode: block all drawing regardless of touch state
	if(m_AndroidLockMode)
	{
		Ui()->ForceMouseButton(0, false);
		Ui()->ForceMouseButton(1, false);
	}
...
#if defined(CONF_PLATFORM_ANDROID)
void CEditor::RenderAndroidTouchToolbar()
{
	Ui()->MapScreen();
	const CUIRect Screen = *Ui()->Screen();

	const float TotalH  = 64.0f;
	const float Margin  = 3.0f;
	const float ToolbarY = Screen.h - TotalH;
	const float BtnH    = TotalH - Margin * 2.0f;

	// ── Pre-compute button rects ──────────────────────────────────────────
	// Layout (left → right):
	//  Lock | Pick | Destr | gap | Undo | Redo | gap | FlipH | FlipV | Rot | gap | - | 1:1 | + | gap | Grid | gap | Save
	const float W_toggle = 56.0f; // toggle buttons
	const float W_zoom   = 44.0f; // zoom triplet
	const float W_save   = 64.0f;

	CUIRect Bar;
	Bar.x = Margin;
	Bar.y = ToolbarY + Margin;
	Bar.w = Screen.w - Margin * 2.0f;
	Bar.h = BtnH;

	auto Next = [&](float W) -> CUIRect {
		CUIRect R;
		Bar.VSplitLeft(W, &R, &Bar);
		Bar.VSplitLeft(Margin, nullptr, &Bar);
		return R;
	};
	auto Gap = [&]() { Bar.VSplitLeft(Margin * 2.0f, nullptr, &Bar); };

	const CUIRect RLock   = Next(W_toggle);
	const CUIRect RPick   = Next(W_toggle);
	const CUIRect RDestr  = Next(W_toggle);
	Gap();
	const CUIRect RUndo   = Next(W_toggle);
	const CUIRect RRedo   = Next(W_toggle);
	Gap();
	const CUIRect RFlipH  = Next(W_toggle);
	const CUIRect RFlipV  = Next(W_toggle);
	const CUIRect RRot    = Next(W_toggle);
	Gap();
	const CUIRect RZoomOut  = Next(W_zoom);
	const CUIRect RZoomReset = Next(W_zoom);
	const CUIRect RZoomIn   = Next(W_zoom);
	Gap();
	const CUIRect RGrid   = Next(W_toggle);
	Gap();
	const bool CanSave    = Map()->m_aFilename[0] != '\0' && Map()->m_ValidSaveFilename;
	const CUIRect RSave   = Next(W_save);

	// ── Touch detection: fire only on new finger-down ─────────────────────
	static std::vector<IInput::CTouchFinger> s_PrevFingers;
	const auto &vFingers = Input()->TouchFingerStates();

	for(const auto &State : vFingers)
	{
		bool IsNew = true;
		for(const auto &Prev : s_PrevFingers)
			if(Prev == State.m_Finger) { IsNew = false; break; }
		if(!IsNew) continue;

		const float Tx = State.m_Position.x * Screen.w;
		const float Ty = State.m_Position.y * Screen.h;
		if(Ty < ToolbarY) continue;

		auto Hit = [&](const CUIRect &R) {
			return Tx >= R.x && Tx <= R.x + R.w && Ty >= R.y && Ty <= R.y + R.h;
		};

		if(Hit(RLock))
			m_AndroidLockMode = !m_AndroidLockMode;
		else if(Hit(RPick))
			m_AndroidPickerForced = !m_AndroidPickerForced;
		else if(Hit(RDestr))
			m_BrushDrawDestructive = !m_BrushDrawDestructive;
		else if(Hit(RUndo) && ActiveHistory().CanUndo())
			ActiveHistory().Undo();
		else if(Hit(RRedo) && ActiveHistory().CanRedo())
			ActiveHistory().Redo();
		else if(Hit(RFlipH))
		{
			for(auto &pLayer : m_pBrush->m_vpLayers)
		...
		{
			for(auto &pLayer : m_pBrush->m_vpLayers)
				pLayer->BrushRotate(pi / 2.0f); // 90° CW
		}
		else if(Hit(RZoomOut))
			m_QuickActionZoomOut.Call();
		else if(Hit(RZoomReset))
			m_QuickActionResetZoom.Call();
		else if(Hit(RZoomIn))
			m_QuickActionZoomIn.Call();
		else if(Hit(RGrid))
		...
		{
			if(CanSave)
				Map()->Save(Map()->m_aFilename,
					[](const char *pErr){ dbg_msg("editor/android", "save: %s", pErr); });
			else
				m_FileBrowser.ShowFileDialog(IStorage::TYPE_SAVE,
					CFileBrowser::EFileType::MAP,
					"Save map", "Save", "maps", "", CallbackSaveMap, this);
		}
	}

	s_PrevFingers.clear();
	for(const auto &State : vFingers)
		s_PrevFingers.push_back(State.m_Finger);
	...
	// Toolbar background
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.07f, 0.07f, 0.07f, 0.90f);
	IGraphics::CQuadItem BgQ(0.0f, ToolbarY, Screen.w, TotalH);
	Graphics()->QuadsDrawTL(&BgQ, 1);
	Graphics()->QuadsEnd();

	// Helper: draw one button
	auto Btn = [&](const CUIRect &R, const char *pText, bool Active, bool Disabled = false)
	{
		Graphics()->QuadsBegin();
		if(Disabled)
			Graphics()->SetColor(0.18f, 0.18f, 0.18f, 0.75f);
		else if(Active)
			Graphics()->SetColor(0.10f, 0.42f, 0.10f, 0.95f); // green = ON
		else
			Graphics()->SetColor(0.30f, 0.30f, 0.30f, 0.95f);
		IGraphics::CQuadItem Q(R.x, R.y, R.w, R.h);
		Graphics()->QuadsDrawTL(&Q, 1);
		Graphics()->QuadsEnd();

		Ui()->DoLabel(&R, pText, 11.0f, TEXTALIGN_MC);
	};

	const bool HasBrush = !m_pBrush->m_vpLayers.empty();

	Btn(RLock,  m_AndroidLockMode    ? "Locked" : "Lock",  m_AndroidLockMode);
	Btn(RPick,  "Pick",  m_AndroidPickerForced);
	Btn(RDestr, "Destr", m_BrushDrawDestructive);

	Btn(RUndo,  "Undo",  false, !ActiveHistory().CanUndo());
	Btn(RRedo,  "Redo",  false, !ActiveHistory().CanRedo());

	Btn(RFlipH, "FlipH", false, !HasBrush);
	Btn(RFlipV, "FlipV", false, !HasBrush);
	Btn(RRot,   "Rot90", false, !HasBrush);

	Btn(RZoomOut,   "-",   false);
	Btn(RZoomReset, "1:1", false);
	Btn(RZoomIn,    "+",   false);

	Btn(RGrid, "Grid", MapView()->MapGrid()->IsEnabled());

	// Save — amber when unsaved changes
	{
		Graphics()->QuadsBegin();
		if(HasUnsavedData())
			Graphics()->SetColor(0.48f, 0.34f, 0.04f, 0.95f);
		else
			Graphics()->SetColor(0.30f, 0.30f, 0.30f, 0.95f);
		IGraphics::CQuadItem Q(RSave.x, RSave.y, RSave.w, RSave.h);
		Graphics()->QuadsDrawTL(&Q, 1);
		Graphics()->QuadsEnd();
		Ui()->DoLabel(&RSave, "Save", 11.0f, TEXTALIGN_MC);
	}
}
#endif
...
void CEditor::Render()
{
...
#if defined(CONF_PLATFORM_ANDROID)
	// Scale editor UI for phone: use 400px logical height instead of 600px
	// → everything (buttons, text, toolbox) appears 1.5× bigger.
	Ui()->SetLogicalHeight(400.0f);
#endif
...
#if defined(CONF_PLATFORM_ANDROID)
	// On Android the tile picker can also be toggled via the touch toolbar button
	if(m_AndroidPickerForced && m_Dialog == DIALOG_NONE && Map()->m_vSelectedLayers.size() == 1)
		m_ShowPicker = true;
#endif
...
#if defined(CONF_PLATFORM_ANDROID)
		// Reserve the bottom 64px for the touch toolbar BEFORE placing StatusBar and
		// ExtraEditor — this way History/ServerSettings panels appear above our toolbar.
		CUIRect AndroidToolbarArea;
		View.HSplitBottom(64.0f, &View, &AndroidToolbarArea);
#endif
	...
	else
	{
		...
		ToolBar.h = 50;
#if defined(CONF_PLATFORM_ANDROID)
		// GUI is hidden: still reserve toolbar area so map doesn't cover it
		View.HSplitBottom(64.0f, &View, nullptr);
#endif
	}
...
#if defined(CONF_PLATFORM_ANDROID)
	// Render Android touch toolbar at the bottom of the screen
	RenderAndroidTouchToolbar();
#endif
