...
void CEditor::DoMapEditor(CUIRect View)
{
	...
	// Lock mode: block all drawing/selection — only zoom/pan gestures work.
...
void CEditor::RenderAndroidTouchToolbar()
{
	...
	const float ToolbarY = Screen.h - TotalH; // top Y of toolbar in logical coords

	// ── Layout: pre-compute all button rects ─────────────────────────────
	const float BtnW  = 72.0f;
	const float ZoomW = 52.0f;
	...
	const CUIRect RLock      = NextBtn(BtnW);
	const CUIRect RPick      = NextBtn(BtnW);
	Gap();
	const CUIRect RZoomOut   = NextBtn(ZoomW);
	const CUIRect RZoomReset = NextBtn(ZoomW);
	const CUIRect RZoomIn    = NextBtn(ZoomW);
	Gap();
	const bool CanSave = Map()->m_aFilename[0] != '\0' && Map()->m_ValidSaveFilename;
	const CUIRect RSave = NextBtn(BtnW);

	// ── Touch detection: fire only on NEW finger-down events ─────────────
	// We track which finger IDs were present last frame.
	// A finger that appears NOW but wasn't present before = new tap.
	...
	{
		// Is this finger new (just pressed down this frame)?
		bool IsNew = true;
		for(const auto &Prev : s_PrevFingers)
		{
			if(Prev == State.m_Finger)
			{
				IsNew = false;
				break;
			}
		...
		// Convert normalised position to logical screen coords
		const float Tx = State.m_Position.x * Screen.w;
		const float Ty = State.m_Position.y * Screen.h;

		if(Ty < ToolbarY)
			continue; // touch is in the map area, ignore

		// Hit test each button
		auto Hit = [&](const CUIRect &R) -> bool
		{
	...
	// Update previous finger list for next frame
	s_PrevFingers.clear();
	for(const auto &State : vFingers)
		s_PrevFingers.push_back(State.m_Finger);

	// ── Rendering: draw buttons manually, no DoButton_Ex ─────────────────
	// This avoids all UI hot/active item complications on touchscreens.

	Graphics()->TextureClear();

	auto DrawBtnBg = [&](const CUIRect &R, bool Active, bool Disabled)
	{
		Graphics()->QuadsBegin();
		if(Disabled)
			Graphics()->SetColor(0.20f, 0.20f, 0.20f, 0.70f);
		else if(Active)
			Graphics()->SetColor(0.15f, 0.45f, 0.15f, 0.92f); // green = ON
		else
			Graphics()->SetColor(0.28f, 0.28f, 0.28f, 0.92f); // grey = OFF
		IGraphics::CQuadItem Q(R.x, R.y, R.w, R.h);
		Graphics()->QuadsDrawTL(&Q, 1);
		Graphics()->QuadsEnd();
	};

	auto DrawBtnText = [&](const CUIRect &R, const char *pText)
	{
		Ui()->DoLabel(&R, pText, 13.0f, TEXTALIGN_MC);
	};

	// toolbar background
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.08f, 0.08f, 0.08f, 0.88f);
	IGraphics::CQuadItem BgQ(0.0f, ToolbarY, Screen.w, TotalH);
	Graphics()->QuadsDrawTL(&BgQ, 1);
	Graphics()->QuadsEnd();

	// Lock
	DrawBtnBg(RLock, m_AndroidLockMode, false);
	DrawBtnText(RLock, m_AndroidLockMode ? "Locked" : "Lock");

	// Pick
	DrawBtnBg(RPick, m_AndroidPickerForced, false);
	DrawBtnText(RPick, "Pick");

	// Zoom out
	DrawBtnBg(RZoomOut, false, false);
	DrawBtnText(RZoomOut, "-");

	// Zoom reset
	DrawBtnBg(RZoomReset, false, false);
	DrawBtnText(RZoomReset, "1:1");

	// Zoom in
	DrawBtnBg(RZoomIn, false, false);
	DrawBtnText(RZoomIn, "+");

	// Save (highlight yellow if unsaved changes)
	{
		const bool Unsaved = HasUnsavedData();
		Graphics()->QuadsBegin();
		if(Unsaved)
			Graphics()->SetColor(0.45f, 0.35f, 0.05f, 0.92f); // amber = unsaved
		else
			Graphics()->SetColor(0.28f, 0.28f, 0.28f, 0.92f);
		IGraphics::CQuadItem Q(RSave.x, RSave.y, RSave.w, RSave.h);
		Graphics()->QuadsDrawTL(&Q, 1);
		Graphics()->QuadsEnd();
		DrawBtnText(RSave, "Save");
	}
}
#endif
