...
{
	// Floating touch toolbar: row of large buttons at the bottom of the screen.
	// Designed for fingers — each button is 56px tall and at least 64px wide.
	// The toolbar is semi-transparent so it does not cover too much map area.
	Ui()->MapScreen();
	...
	const float TotalH = BtnH + Margin * 2.0f;
	...
	Graphics()->TextureClear();
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.1f, 0.1f, 0.1f, 0.72f);
	...
	// Button layout: Undo | Redo | Save | New | ZoomOut | ResetZoom | ZoomIn | ToggleGrid | GUI
	struct SAndroidBtn
	{
		const void *m_pId;
		const char *m_pLabel;
	...
	static int s_UndoBtn = 0, s_RedoBtn = 0, s_SaveBtn = 0, s_NewBtn = 0;
	static int s_ZoomOutBtn = 0, s_ResetZoomBtn = 0, s_ZoomInBtn = 0;
	static int s_GridBtn = 0, s_GuiBtn = 0, s_PickerBtn = 0;

	const float BtnW = 72.0f;
	const float SmallBtnW = 56.0f;

	CUIRect Btn;

	// Undo
	Bar.VSplitLeft(BtnW, &Btn, &Bar);
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	{
		const bool Disabled = !ActiveHistory().CanUndo();
		const int Color = Disabled ? -1 : 0;
		if(DoButton_Ex(&s_UndoBtn, "↩ Undo", Color, &Btn, BUTTONFLAG_LEFT, "Undo last action", IGraphics::CORNER_ALL, 12.0f) && !Disabled)
			ActiveHistory().Undo();
	}
	...
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	{
		const bool Disabled = !ActiveHistory().CanRedo();
		const int Color = Disabled ? -1 : 0;
		if(DoButton_Ex(&s_RedoBtn, "↪ Redo", Color, &Btn, BUTTONFLAG_LEFT, "Redo last undone action", IGraphics::CORNER_ALL, 12.0f) && !Disabled)
			ActiveHistory().Redo();
	}
	...
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	{
		...
		const int Color = HasUnsavedData() ? 1 : (CanSave ? 0 : -1);
		if(DoButton_Ex(&s_SaveBtn, "💾 Save", Color, &Btn, BUTTONFLAG_LEFT, "Save map (Ctrl+S)", IGraphics::CORNER_ALL, 12.0f))
		{
			if(CanSave)
			{
				// Direct save when file already has a valid name
				Map()->Save(Map()->m_aFilename, [](const char *pErr){ dbg_msg("editor/android", "save error: %s", pErr); });
			}
			else
			{
	...
	// Separator gap
	Bar.VSplitLeft(Margin * 3.0f, nullptr, &Bar);

	// Zoom out
	Bar.VSplitLeft(SmallBtnW, &Btn, &Bar);
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	if(DoButton_Ex(&s_ZoomOutBtn, "−", 0, &Btn, BUTTONFLAG_LEFT, "Zoom out", IGraphics::CORNER_L, 18.0f))
		m_QuickActionZoomOut.Call();

	// Reset zoom
	Bar.VSplitLeft(SmallBtnW, &Btn, &Bar);
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	if(DoButton_Ex(&s_ResetZoomBtn, "⊙", 0, &Btn, BUTTONFLAG_LEFT, "Reset zoom", IGraphics::CORNER_NONE, 16.0f))
		m_QuickActionResetZoom.Call();

	// Zoom in
	Bar.VSplitLeft(SmallBtnW, &Btn, &Bar);
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	if(DoButton_Ex(&s_ZoomInBtn, "+", 0, &Btn, BUTTONFLAG_LEFT, "Zoom in", IGraphics::CORNER_R, 18.0f))
		m_QuickActionZoomIn.Call();
	...
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	{
		const int GridActive = MapView()->MapGrid()->IsEnabled() ? -1 : 0;
		if(DoButton_Ex(&s_GridBtn, "⊞ Grid", GridActive, &Btn, BUTTONFLAG_LEFT, "Toggle grid (Ctrl+G)", IGraphics::CORNER_ALL, 11.0f))
			m_QuickActionToggleGrid.Call();
	}
	...
	Bar.VSplitLeft(Margin, nullptr, &Bar);
	{
		const int PickerActive = m_AndroidPickerForced ? -1 : 0;
		if(DoButton_Ex(&s_PickerBtn, "🖌 Pick", PickerActive, &Btn, BUTTONFLAG_LEFT, "Toggle tile picker", IGraphics::CORNER_ALL, 11.0f))
			m_AndroidPickerForced = !m_AndroidPickerForced;
	}

	// Toggle GUI (hide toolbar to see more map)
	{
		// Right-align
		CUIRect GuiBtn;
		GuiBtn.x = Screen.w - Margin - BtnW;
		GuiBtn.y = Bar.y;
		GuiBtn.w = BtnW;
		GuiBtn.h = Bar.h;
		if(DoButton_Ex(&s_GuiBtn, "👁 GUI", m_GuiActive ? -1 : 0, &GuiBtn, BUTTONFLAG_LEFT, "Toggle GUI visibility", IGraphics::CORNER_ALL, 11.0f))
