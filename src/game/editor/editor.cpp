...
void CEditor::RenderAndroidTouchToolbar()
{
	...
	// Toolbar height must match AndroidToolbarH constant used in DoMapEditor & Render()
	const float TotalH = 64.0f; // = BtnH(56) + Margin*2(4+4)
	const float BtnH = 56.0f;
	const float Margin = 4.0f;

	CUIRect Bar;
	Bar.x = 0.0f;
	Bar.y = Screen.h - TotalH;
	Bar.w = Screen.w;
	Bar.h = TotalH;
	...
	Graphics()->TextureClear();
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.08f, 0.08f, 0.08f, 0.85f);
	IGraphics::CQuadItem QuadItem(Bar.x, Bar.y, Bar.w, Bar.h);
	Graphics()->QuadsDrawTL(&QuadItem, 1);
	Graphics()->QuadsEnd();
	...
	// Button IDs
	static int s_LockBtn, s_PickBtn;
	static int s_ZoomOutBtn, s_ZoomResetBtn, s_ZoomInBtn;
	static int s_UndoBtn, s_RedoBtn, s_SaveBtn;

	const float BtnW   = 72.0f;
	const float ZoomW  = 52.0f;
	const float SmallW = 64.0f;

	CUIRect Btn;

	auto Split = [&](float W)
	{
		Bar.VSplitLeft(W, &Btn, &Bar);
		Bar.VSplitLeft(Margin, nullptr, &Bar);
	};
	auto Gap = [&]() { Bar.VSplitLeft(Margin * 2.0f, nullptr, &Bar); };

	// ── 🔒 Lock ────────────────────────────────────────────────────────────
	// When active: drawing is blocked, only two-finger zoom/pan works.
	Split(BtnW);
	if(DoButton_Ex(&s_LockBtn, m_AndroidLockMode ? "🔒" : "🔓",
		m_AndroidLockMode ? -1 : 0, &Btn,
		BUTTONFLAG_LEFT, "Lock canvas (zoom/pan only)", IGraphics::CORNER_ALL, 18.0f))
		m_AndroidLockMode = !m_AndroidLockMode;

	// ── 🖌 Pick ────────────────────────────────────────────────────────────
	Split(BtnW);
	if(DoButton_Ex(&s_PickBtn, "Pick",
		m_AndroidPickerForced ? -1 : 0, &Btn,
		BUTTONFLAG_LEFT, "Toggle tile picker", IGraphics::CORNER_ALL, 13.0f))
	...
	// ── Zoom − / 1:1 / + ──────────────────────────────────────────────────
	Split(ZoomW);
	if(DoButton_Ex(&s_ZoomOutBtn, "−", 0, &Btn,
		BUTTONFLAG_LEFT, "Zoom out", IGraphics::CORNER_L, 20.0f))
		m_QuickActionZoomOut.Call();

	Split(ZoomW);
	if(DoButton_Ex(&s_ZoomResetBtn, "1:1", 0, &Btn,
		BUTTONFLAG_LEFT, "Reset zoom", IGraphics::CORNER_NONE, 11.0f))
		m_QuickActionResetZoom.Call();

	Split(ZoomW);
	if(DoButton_Ex(&s_ZoomInBtn, "+", 0, &Btn,
		BUTTONFLAG_LEFT, "Zoom in", IGraphics::CORNER_R, 20.0f))
		m_QuickActionZoomIn.Call();
	...
	Split(SmallW);
	{
		const bool CanUndo = ActiveHistory().CanUndo();
		if(DoButton_Ex(&s_UndoBtn, "↩", CanUndo ? 0 : -1, &Btn,
			BUTTONFLAG_LEFT, "Undo (Ctrl+Z)", IGraphics::CORNER_L, 18.0f) && CanUndo)
			ActiveHistory().Undo();
	}
	Split(SmallW);
	{
		const bool CanRedo = ActiveHistory().CanRedo();
		if(DoButton_Ex(&s_RedoBtn, "↪", CanRedo ? 0 : -1, &Btn,
			BUTTONFLAG_LEFT, "Redo (Ctrl+Y)", IGraphics::CORNER_R, 18.0f) && CanRedo)
			ActiveHistory().Redo();
	}
	...
	Split(BtnW);
	{
		...
		const int Color = HasUnsavedData() ? 1 : 0;
		if(DoButton_Ex(&s_SaveBtn, "Save", Color, &Btn,
			BUTTONFLAG_LEFT, "Save map", IGraphics::CORNER_ALL, 13.0f))
		{
			if(CanSave)
				...
				m_FileBrowser.ShowFileDialog(IStorage::TYPE_SAVE,
					CFileBrowser::EFileType::MAP, "Save map", "Save",
					"maps", "", CallbackSaveMap, this);
		}
	}
}
#endif
