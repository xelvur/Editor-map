...
void CMapView::HandleTouchGestures()
{
	const auto &vFingers = Input()->TouchFingerStates();
	const int NumFingers = (int)vFingers.size();

	const float ScreenW = Ui()->Screen()->w;
	const float ScreenH = Ui()->Screen()->h;
	// Use only the non-toolbar part of the screen for gesture calculations
	const float EffectiveH = ScreenH - 64.0f;

	if(NumFingers >= 2)
	{
		// Use only the first two fingers
		const vec2 Pos0 = vFingers[0].m_Position * vec2(ScreenW, ScreenH);
		const vec2 Pos1 = vFingers[1].m_Position * vec2(ScreenW, ScreenH);

		// Ignore touches in the toolbar area
		if(Pos0.y > EffectiveH || Pos1.y > EffectiveH)
		{
			m_TouchPinchActive = false;
			m_TouchPanActive   = false;
			return;
		}

		const vec2 Center = (Pos0 + Pos1) * 0.5f;
		const float Dist  = distance(Pos0, Pos1);

		if(!m_TouchPinchActive)
		{
			// First frame of two-finger gesture — record baseline, no movement yet.
			// This prevents the "jump" when a second finger first appears.
			m_TouchPinchActive     = true;
			m_TouchPanActive       = true;
			m_TouchPinchInitialZoom = m_Zoom.GetValue();
			m_TouchPinchInitialDist = (Dist > 4.0f) ? Dist : 4.0f;
			m_TouchPanLastCenter   = Center;
		}
		else
		{
			// ── Pan: move by center delta ────────────────────────────────
			const vec2 CenterDelta = Center - m_TouchPanLastCenter;

			// Only apply pan if delta is large enough to be intentional
			// (avoids micro-jitter from finger wobble)
			if(length(CenterDelta) > 0.3f)
			{
				OffsetWorld(-CenterDelta * Editor()->m_MouseWorldScale);
			}
			m_TouchPanLastCenter = Center;

			// ── Pinch-to-zoom: relative to initial distance ──────────────
			// Re-anchor the zoom reference each frame so zoom feels
			// natural and avoids accumulating error.
			if(Dist > 4.0f && m_TouchPinchInitialDist > 4.0f)
			{
				const float Ratio = m_TouchPinchInitialDist / Dist;
				const float MaxZoom = g_Config.m_EdLimitMaxZoomLevel ? 2000.0f : 50000.0f;
				const float NewZoom = std::max(10.0f, std::min(MaxZoom,
					m_TouchPinchInitialZoom * Ratio));

				// Only update if change is meaningful (avoids micro-zoom jitter)
				if(std::fabs(NewZoom - m_Zoom.GetValue()) > 0.5f)
					m_Zoom.SetValueInstant(NewZoom);

				// Re-anchor every frame so zoom tracks live distance,
				// not the original touch-down distance
				m_TouchPinchInitialZoom = m_Zoom.GetValue();
				m_TouchPinchInitialDist = Dist;
			}
		}
	}
	else
	{
		// No two-finger gesture active
		m_TouchPinchActive = false;
		m_TouchPanActive   = false;
	}
}
