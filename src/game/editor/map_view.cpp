...
void CMapView::HandleTouchGestures()
{
	...
	if(NumFingers == 2)
	{
		// Compute positions in screen coords
		...
		if(!m_TouchPinchActive && !m_TouchPanActive)
		{
			// Start of two-finger gesture
			m_TouchPinchActive = true;
			m_TouchPanActive = true;
			m_TouchPinchInitialZoom = m_Zoom.GetValue();
			m_TouchPinchInitialDist = (Dist > 1.0f) ? Dist : 1.0f;
			m_TouchPanLastCenter = Center;
		}
		else
		{
			// Panning: move by center delta
			const vec2 CenterDelta = Center - m_TouchPanLastCenter;
			OffsetWorld(-CenterDelta * Editor()->m_MouseWorldScale);
			m_TouchPanLastCenter = Center;

			// Pinch-to-zoom: scale zoom by dist ratio
			if(m_TouchPinchInitialDist > 1.0f && Dist > 1.0f)
			{
				const float NewZoom = m_TouchPinchInitialZoom * (m_TouchPinchInitialDist / Dist);
				// Clamp to valid range
				const float ClampedZoom = std::max(10.0f, std::min(g_Config.m_EdLimitMaxZoomLevel ? 2000.0f : 50000.0f, NewZoom));
				m_Zoom.SetValue(ClampedZoom);
			}
		}
	}
	else
	{
		// Reset two-finger gesture state when not exactly 2 fingers
