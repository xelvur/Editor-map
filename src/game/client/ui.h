...
private:
	...
	CUIRect m_Screen;
	float m_LogicalHeight = 600.0f; // Default logical height; lower = larger UI
...
public:
	...
	// Android: forcibly override a mouse button state for this frame (call before editor processes input)
	void ForceMouseButton(int Index, bool Pressed)
	{
		if(Pressed) m_MouseButtons |= (1u << Index);
		else m_MouseButtons &= ~(1u << Index);
	}
	// Scale the logical screen height (default 600). Lower = everything appears larger.
