...
std::unique_ptr<CTouchControls::CTouchButtonBehavior> CTouchControls::ParseBehavior(const json_value *pBehaviorObject)
{
	...
	else if(str_comp(BehaviorType.u.string.ptr, CSimulateKeyTouchButtonBehavior::BEHAVIOR_TYPE) == 0)
	{
		auto pBehavior = std::make_unique<CSimulateKeyTouchButtonBehavior>();
		const json_value &Key = BehaviorObject["key"];
		if(Key.type == json_string)
		{
			str_copy(pBehavior->m_aKeyName, Key.u.string.ptr, sizeof(pBehavior->m_aKeyName));
